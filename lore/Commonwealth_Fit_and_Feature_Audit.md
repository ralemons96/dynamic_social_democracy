...
3. **F11 systems depth — ✎ DONE (first tranche):** magi axis now has three movers (the card + Harnfell's new "Advocacy for the magi" advisor action + the new **Circles Petition** event); `bvp_relation` has real consumers (petition trigger/aside + the Avriza majority beat); the new **An Envoy from the North** event narrates empire_posture/reunification/fear bands post-Ivion. Further depth (more Empire beats, dufour_faith surfacing) remains open for a later tranche. ✎ ______
   **Tranche 2 (user-directed, 2026-08-13):** Imperial Relations gains **@moderates** (stackable +5 posture lever) · the magi card gains repeatable **@crackdown** (pleases Golds/Unionists, −5 posture, −8 magi_balance; card stays available to −20 balance) · Defense Appropriations gains **@appropriate_major** (2 budget → 1.5× strength, base-anger costs) · **unemployment now drags military_readiness** (−3%/pt above disfavored 12, floor −45%, both formula copies) · Golds start +7 (42 main / 38 historical) · NEW event **"Naval Action off Stathmore!"** (Ivion+7mo; observe/retreat/fight × readiness bands weak<130 / avg 130–249 / strong≥250 — six outcomes, all engine-verified).
