...
## ⏳ Deferred to next pass (your holds + blanks + new finds)
1. **F6 `popular_front_dispute` (your hold) — investigated:** it is wired ONLY as a timer name in `Q.timers`/`Q.rubicon_timers` (root:743,769); the event itself is `#tags`-disabled, unreachable, and empty. Fully inert as-is. Options: delete (file + the 2 list entries) or write it as a real Popular-Front dispute event. ✎ ______
2. **F12 content stubs (left blank):** schleicher_reichstag_7 two-line climax · papen_joever skeleton+TODO · harzburg @strategy TODO · nazi_10/11 missing outcomes · kpd_policy half-built-disabled · nazis_in_crisis dead twin · kpd_ultimatum "We were so kind."/groko "Damn." one-liners · schleicher_cabinet_8 twin bank bodies. *(The `_12_alt` empty-choice softlock (g) WAS fixed — it was a bug, not content.)* ✎ ______
3. **F11 shallow axes (your "larger backlog item"):** magi_balance single-lever (0.25 of reunification), bvp_relation dead, Empire-axis legibility before the Convention. ✎ ______
4. **F8a `staal_parliament_trigger`:** investigated — never written even in the BASE game (upstream-dead choose-if). Needs a design call: build the event where Staal accepts parliamentary backing, or delete the greyed option. ✎ ______
5. **F3 remainder:** 13 legacy-granted-but-undisplayed achievement ids (`ddp_*`/`dstp_*` family, `constitutional_coalition`, `reichskonkordat`, `stolper_program`) — inert; add to the display array with coined titles, or strip the grants. ✎ ______
6. **F9d remainder:** `bourgeois_merger_timer`/`papen_chancellor_timer` assignments left in place — they are NOT in the tick lists (inert one-line writes, zero cost). ✎ ______
