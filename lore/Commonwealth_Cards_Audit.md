...
- **"Ironreach" is an established locale in events** — `labor_unrest` ("the mining and metals industry of the Ironreach region") and `dvp_party_congress_3_luther` ("Ironreach heavy industry"). It is **not** a real-world residual — it's a coined industrial-heartland (Ruhr-analogue) used coherently. The card (`labor_affairs`) now says **Abysm**, so the same region reads under two names — and Abysm is elsewhere a distinct western locale. **DECISION:** revert the card to Ironreach (keep it as the canon industrial region), or sweep Ironreach→Abysm game-wide (merging them)?
